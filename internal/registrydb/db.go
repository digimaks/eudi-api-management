// Package registrydb calls the registry-schema SECURITY DEFINER procedures
// via the JSONB envelope. It never touches registry tables directly: it
// connects as management_public, an EXECUTE-only role with no table or
// sequence grants.
package registrydb
...
// here so `registry:not_found` surfaces as err:registry:not_found (404)
// rather than an opaque 500. An empty code is not an error.
